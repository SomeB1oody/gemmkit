...
The three block sizes keep the `A` and `B` panels resident, but they say nothing about `C`: every microtile call read-modify-writes its `mr x nr` output tile, and once a call's working set - the `A`, `B`, and `C` bytes together - outgrows the per-core-reachable LLC, that output tile no longer lives in cache and its store reaches into DRAM. The driver answers with a software prefetch, decided once per call: it compares the working set against `cache::prefetch_ws_bytes` (the `GEMMKIT_PREFETCH_MIN_BYTES` gate, `0` = auto = the per-core-reachable LLC, L3 where present, else L2), and when the gate clears it issues a T0 prefetch of each output microtile just ahead of that tile's microkernel call, pulling the lines the store will touch into L1 while the microkernel still computes. The hint walks whole 64-byte cache lines along the tile's unit-stride dimension (a tile strided in both dimensions has no contiguous lines and is skipped) and is emitted only on `x86_64` (`prefetcht0`, baseline SSE, needs no feature gate); on any other target it lowers to nothing, so aarch64 and wasm are untouched. It moves cache lines only, never arithmetic, so results are bit-identical with the gate on, off, or forced. Measured on the Zen5 9950X (`f32`, AVX-512F, 32 MiB per-CCD LLC): +1.4% parallel and about +1% serial at 2048^3 (a 48 MiB working set), +3.0% at 3072^3, and +2.6% at the deep-k 2048x2048x24576 shape (both parallel); below the gate, where the tiles are cache-resident, the path is unchanged and measures as noise.
...
The backends, in order: **CPUID** (`cache/cpuid.rs`) is an instruction, so it works regardless of OS, in containers and most VMs - both vendors are read through the per-cache topology leaf (Intel `04h`, AMD `0x8000_001D`), which describes each cache *as reachable from the executing core*: on a multi-die part such as a 2-CCD Ryzen the L3 figure is the one complex a core can actually hit (32 MiB on the 9950X), not the package total, which is the semantic every consumer of the value wants. AMD parts or hypervisors without that leaf fall back to the legacy L1 (`0x8000_0005`) and L2/L3 (`0x8000_0006`) leaves, where L3 size arrives in units of 512 KiB as a package total and 16-way associativity is not even encodable. **Linux sysfs** (`cache/sysfs.rs`) parses `/sys/devices/system/cpu/cpu0/cache/index*/` with plain `std::fs` - a fallback on x86 Linux (a hypervisor that masks CPUID), the primary source on aarch64 Linux, which has no CPUID instruction. **macOS sysctl** (`cache/sysctl.rs`) reads `sysctlbyname` keys through a two-line `extern "C"` block (no `libc` dependency), preferring the Apple Silicon per-performance-level keys (`hw.perflevel0.*`, the P-cores) with the flat Intel-Mac keys as fallback; `sysctl` does not expose associativity, so conservative typical values are assumed - safe because the model clamps associativity with `.max(2)` and needs it only approximately. The bottom of the chain is `ZEN5_FALLBACK`, a static default calibrated on the Ryzen 9950X dev machine: 48 KiB / 12-way L1d, 1 MiB / 16-way private L2, 32 MiB / 16-way L3.
