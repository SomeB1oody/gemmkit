...
**`FusedEpi`** is the runtime-composed bias-plus-activation epilogue: a per-row or per-column bias (`Bias::PerRow` / `Bias::PerCol`), then `Relu` or `LeakyRelu(slope)`. One monomorphization covers every combination, so the fused kernel count is not multiplied by the number of epilogue kinds — but only because the enum branches are decoded *once per tile*, in the `apply_tile` override, rather than once per accumulator. That distinction is not a micro-optimization. The kernel's store pass is unrolled by the tile const generics, so a per-register hook replicates both matches in every one of the tile's accumulator slots, and on a wide tile the resulting branch web costs the compiler the accumulator tile itself: it stops keeping `acc` in registers and writes every value through to the stack from inside the `kc` loop, where the epilogue does not even run. On the 24-accumulator AVX-512 `f32` tile that was 24 extra stores per `kc` iteration and a fused GEMM at one third the plain rate, for an epilogue whose real cost is immeasurable. Hoisting the decode restores it exactly; `tests/perf/fused.rs` pins the ratio so it cannot silently come back. It backs `gemm_fused` and its whole constellation: `gemm_batched_fused` (one shared bias and activation across the batch), the prepacked twins `gemm_packed_b_fused` / `gemm_packed_a_fused`, and the complex entry `gemm_cplx_fused`, which is bias-only because an ordering-based activation is mathematically undefined on complex numbers. It sets `VECTOR = true`: on the fast path the bias add and activation run as register operations (`max(v, 0)` and friends), with a NaN contract on the SIMD `max`/`min` chosen so the vector and scalar forms agree exactly (`ReLU(NaN) = 0` on both).
...
3. **Bit-agreeing apply paths.** A full column-major tile stores through the vector path (`apply_tile`, itself defaulting to `apply_reg`, or `apply_store`); an edge or strided tile drains through scratch and the scalar `apply`; a single output matrix mixes the two freely — so the trait contract requires them to agree bit-for-bit under the same token. An `apply_tile` override inherits that obligation: it must leave exactly what `apply_reg` would have, element for element.
