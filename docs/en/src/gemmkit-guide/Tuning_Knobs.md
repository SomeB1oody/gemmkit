...
| `GEMMKIT_GEMV_PARALLEL_BYTES` | `set_gemv_parallel_bytes` | 0 (auto) | Byte floor below which a bandwidth-bound gemv/gevv stays single-threaded (below it the touched data is LLC-resident and splitting only loses). `0` derives it from the LLC size: half the per-core-reachable L3, or on an L3-less aarch64 part an eighth of the shared cluster L2 (2 MiB on the M4 Max, where parallel wins from a 3.2 MiB matrix once the tier pools cut the fork cost). |
