...
| `GEMMKIT_LHS_PACK_REUSE` | `set_lhs_pack_reuse` | 128 (aarch64: 4) | Reuse floor that prices the force-pack's benefit rather than its cost: the stride and span gates above only fire when at least this many `nr`-wide column tiles reuse each packed panel (`min(n, nc) / nr`, rounded up). A tall/skinny shape (`m` much greater than `n`) has a huge span but few column tiles, so it would amortize an expensive pack over too little reuse; `0` drops the floor and lets the stride+span pair decide alone. On aarch64 the trade nearly inverts - packing is cheap and the in-place walk strides 16 KiB pages - so the M4 Max wants the pack from 4 tiles of reuse up (`m = k = 4096`: `n = 32` +88%, `n = 256` +220%) and only `n_nt = 2` still prefers in-place. |
| `GEMMKIT_SHARED_LHS_MNK` | `set_shared_lhs_mnk` | 8e9 (aarch64: 6e6; 32-bit: disabled) | `m*n*k` gate for the shared-A pre-pass on the parallel packed path, which removes redundant per-worker packs at the cost of a fork-join barrier. On the M4 Max the shared pass wins from 8.4e6 up (+66% there, +39% at 3.4e7) and only 4.2e6 still prefers per-worker, hence the far lower aarch64 gate. Independent of this gate, the pre-pass also opens from 16 workers up, where the per-worker redundancy always outweighs the barrier. |
...
| `GEMMKIT_SMALL_MN_DIM` | `set_small_mn_dim` | 16 (aarch64: 32) | Both `m` and `n` at or below this (with a long `k`) take the horizontal inner-product route, where each output is one SIMD-reduced dot. `0` disables the route. On the M4 Max the route's zero-copy tier dominates through 32 (`32^2 x 65536` mixed layout: 529 vs 74 GFLOP/s against the driver), hence the higher aarch64 cap. |
...
| `GEMMKIT_GEMV_PARALLEL_BYTES` | `set_gemv_parallel_bytes` | 0 (auto) | Byte floor below which a bandwidth-bound gemv/gevv stays single-threaded (below it the matrix fits one core's private cache, which that core saturates alone, so splitting only loses). `0` derives it from the cache: the per-core private L2 on parts with an L3 (1 MiB on the Zen5 9950X), or on an L3-less aarch64 part an eighth of the shared cluster L2 (2 MiB on the M4 Max, where parallel wins from a 3.2 MiB matrix once the tier pools cut the fork cost). |
| `GEMMKIT_GEMV_TIER_STEP` | `set_gemv_tier_step` | 0 (auto) | Byte spacing between the rungs of the auto gemv/gevv worker ladder: the width climbs one exact-fit pool tier per factor of this in bytes touched, starting from the byte floor above. `0` means 8, measured on the 9950X, where a repeatedly scanned matrix is fastest at 8 workers up through about 8 MiB and at 16 from about 12 MiB. `1` collapses the ladder onto its top tier. Inert with fewer than 2 pool tiers active, so it does nothing under the single-tier aarch64 default. |
| `GEMMKIT_GEMV_AXPY_PAR_MIN_ROWS` | `set_gemv_axpy_par_min_rows` | 16384 (x86), 1024 (aarch64) | Output-row floor below which a **column-major** gemv stays serial instead of splitting its rows. For a column-major matrix the output-row axis is the inner memory axis, so a split gives every worker a strided walk over the whole matrix while the serial route makes one sequential pass; measured on the 9950X that split loses at every row count below 16384 (0.61-0.90x) and only pays above it (1.14x at 16384, 1.40x at 1M). On the M4 Max the same regression is much narrower: it loses below 1024 rows (0.35-0.94x at 128-512 rows, 0.60-1.22x at 768) and wins from 1024 up (1.39-2.37x). `0` disables the floor. On the M4 Max the crossover tracks *bytes per column* (4 KiB) rather than the row count, so an `f64`-dominated workload on that part should set this to 512. A row-major gemv and the `half` mixed twin are never gated: both measured faster split at every size. |
| `GEMMKIT_GEMV_THREAD_CAP` | `set_gemv_thread_cap` | 0 (auto) | A flat worker count for a bandwidth-bound gemv/gevv, replacing the ladder above outright - the "I measured this machine, use exactly this" override. `0` keeps the ladder, which tops out at half the logical cores since a gemv saturates its bandwidth far below the full width. |
...
| `GEMMKIT_POOL_CLASSES` | `set_pool_classes` | 2 (aarch64: 1; elsewhere: 0) | Number of halving tiers below full machine width - half, then quarter - for which gemmkit keeps a private, persistent rayon pool, built lazily on first use (about 12 microseconds per thread) and never rebuilt. The auto path snaps its worker count exactly to a tier so no thread sits idle at the fork/join barrier; an explicit `Rayon(n)` still gets exactly `n` workers and merely runs in the smallest tier pool that fits. `0` disables tier pools, leaving every call on the ambient pool. Clamped to 3. Defaults to 2 tiers on x86_64 and 1 on aarch64 (on the M4 Max the half-width tier of 7 wins `128^3`-`224^3` by 2.1-2.6x, and a second, smaller tier would misroute those shapes); off elsewhere pending on-device validation. |
| `GEMMKIT_FULL_WIDTH_MNK` | `set_full_width_mnk` | 0 (auto) | The `m*n*k` above which the auto path leaves its largest tier pool for full machine width, once the extra full-width workers finally pay for the fork/join they add; below it, auto stays on its largest tier even though more cores exist. `0` derives it per architecture: 110_000_000 on x86 (calibrated on the 9950X), 14_000_000 on aarch64 (calibrated on the M4 Max, where full width - engaging the E-cores - wins from `256^3` up). `MAX` pins the auto path to the largest tier unconditionally, so full width never engages. |
