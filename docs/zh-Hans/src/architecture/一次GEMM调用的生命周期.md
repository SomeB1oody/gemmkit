...
- **一份扁平的一维作业列表**——每个深度切片内，剩下的工作是 `n_mc` 个行块乘 `n_nt` 个列 tile，压平成 `n_jobs = n_mc * n_nt` 个下标。工作线程按需从共享的无锁 `JobCursor` 拉取连续区块：没有静态划分，快核自然吸收更多工作，区块粒度对线程数过采样（`job_grain`；打包 LHS 路径用与行块对齐的 `packed_block_grain`，区块永不跨越打包边界）。线程数本身来自 `par.resolve(m*n*k, n_jobs)`——感知工作量，随问题规模渐进增长而不是一步跳到全部核心。
