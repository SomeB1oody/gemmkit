...
面向 [gemmkit](https://crates.io/crates/gemmkit) GEMM 引擎的零拷贝 [faer](https://crates.io/crates/faer) 适配器。它接受 faer 的视图类型：输入用 `MatRef<'_, T>`，输出用 `MatMut<'_, T>`。每个入口直接从视图中读出数据指针，以及以元素为单位的 `isize` 行步长和列步长。然后再转交给 gemmkit 的底层引擎。因此 faer 的列主序布局、转置视图、子矩阵以及反转（负步长）视图都无需拷贝即可工作。

各入口点镜像了 gemmkit 的核心 API，包括 feature 门控的元素类型族（`half`、`complex`、`int8`）以及融合尾部运算（fused epilogue）入口（`epilogue`）。faer 没有三维数组类型。批量 GEMM（`gemm_batched`）改为以每个批次元素的 `(A, B)` `MatRef` 输入切片，搭配 `&mut C` `MatMut` 输出切片的形式接收批次。这走的是 gemmkit 的指针数组批量引擎。批次中的每个元素可以有不同的形状。
...
- 融合尾部运算（`epilogue` feature）：`gemm_fused` 一趟算出 `C <- act(alpha*A*B + beta*C + bias)`。`gemm_map` 对每个输出元素应用用户闭包（`f32` / `f64`）。`gemm_i8_requant` 和 `gemm_i8_requant_u8` 对 `i8` 结果做重量化（需配合 `int8`）。`gemm_cplx_fused` 为复数结果加上偏置（需配合 `complex`）。预打包的各入口同样有对应的融合版本。
...
实数 `f32` 与 `f64` 路径始终构建，其余类型由上面的 feature 控制。适配器直接从 faer
的视图读出 `T`，因此下面每种类型都以 faer 原生的 `c32` / `c64`、`f16` / `bf16` 写法
工作，无需任何转换。
