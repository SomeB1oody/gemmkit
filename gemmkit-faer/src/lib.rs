...
//! bias-only [`gemm_cplx_fused`]/[`gemm_cplx_fused_with`] (no activation — undefined on `ℂ`). Like
//! the plain entries, these read raw parts out of the view and forward to gemmkit's raw engine, so
//! transposed, sub-matrix, and reversed (negative-stride) views all work without copying.
//!
...
/// arithmetic only** — no reference is ever formed over the (possibly gappy — a `Mat` whose column
/// stride exceeds `nrows` has uninitialized padding — or exclusively-borrowed) span, which is exactly
...
/// `C <- act(alpha·A·B + beta·C + bias)` in one fused pass — the faer adapter over gemmkit's
...
/// an `i8` output in one pass — the faer adapter over gemmkit's [`gemmkit::gemm_i8_requant`]. The
...
/// forwards to gemmkit's raw engine, so transposed, sub-matrix, and reversed (negative-stride) views
...
/// `conj_a` (resp. `B̅` when `conj_b`) — the faer adapter over gemmkit's
