...
| `wasm_threads` | 否 | 为 `wasm32-wasip1-threads` 启用 `parallel` 和 `gemmkit/wasm_threads`。 |
...
nalgebra 矩阵读出，因此列主序（nalgebra 的自然布局）、行主序以及一般步长的视图都无需
转换即可工作。
...
（`gemm_packed_a` / `gemm_packed_b`）与批量（`gemm_batched`，以每个批次元素的输入切片
形式）路径也承载同样的元素类型。
