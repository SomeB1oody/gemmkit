...
/// Pre-packs a RHS `B` into a reusable [`PackedRhs`], gemmkit's fixed-weight reuse path
///
/// Pack `B` once here, then skip the per-call repack across many [`gemm_packed_b`] calls that
/// share this `B`. This reads `B`'s pointer and strides directly, so any layout works without
/// copying
...
/// `C <- alpha*A*B + beta*C`, reusing a prepacked `B` from [`prepack_rhs`]
///
/// `C` must be column-major-ish (`|col stride| >= |row stride|`). A row-major `C` would force
/// the engine to swap the `A`/`B` roles, which would invalidate the prepacked RHS, so gemmkit
/// rejects it. Use [`gemm`] for a row-major `C`
///
/// # Panics
///
...
/// pool. This suits an inference loop with fixed weights packed once and a stream of activation
/// batches
///
/// # Panics
///
    ...
    SC: RawStorageMut<T, RC, CC>,
{
    ...
    let cp = c.as_mut_ptr();
    // SAFETY: nalgebra guarantees A/C layouts are valid in-bounds. `c` (a `&mut` borrow) cannot
...
/// Pre-packs an LHS `A` into a reusable [`PackedLhs`], for a fixed `A` multiplied against a
/// stream of right-hand operands
///
/// Pack `A` once, then skip the per-call repack across many [`gemm_packed_a`] calls. This reads
/// `A`'s pointer and strides directly, so any layout works without copying
...
/// `C <- alpha*A*B + beta*C`, reusing a prepacked `A` from [`prepack_lhs`]
///
/// `C` must be row-major-ish (`|col stride| <= |row stride|`). A column-major `C` would force
/// the engine to keep `A` in the LHS role rather than swap it. This would invalidate the
/// prepacked LHS, so gemmkit rejects it. Use [`gemm`] for a column-major `C`
///
/// # Panics
///
    ...
    SC: RawStorageMut<T, RC, CC>,
{
    ...
    let (rsc, csc) = (cs.0 as isize, cs.1 as isize);
    let cp = c.as_mut_ptr();
    // SAFETY: nalgebra guarantees B/C layouts are valid in-bounds. `c` (a `&mut` borrow) cannot
...
/// from [`prepack_rhs`]
///
/// This is the nalgebra adapter over gemmkit's [`gemmkit::gemm_packed_b_fused`]. The same
/// [`PackedRhs`] serves both [`gemm_packed_b`] and this fused entry, because the epilogue only
/// changes how the result is stored, not how `B` was packed. `C` must be column-major-ish
...
/// # Panics
///
/// If the dimensions disagree, if `C` is not column-major-ish, or on a bias or activation the
/// adapter rejects: a wrong-length bias, a bias slice that overlaps `C`, or a non-finite
/// `LeakyRelu` slope
#[cfg(feature = "epilogue")]
#[allow(clippy::too_many_arguments)]
    ...
    SC: RawStorageMut<T, RC, CC>,
{
    ...
    // checked entry's wording. The packed-B path never swaps A/B, so the bias stays in the
    // user's frame and needs no axis flip before it is passed down
    ...
    // SAFETY: dims are checked above. nalgebra guarantees A/C layouts are valid in-bounds, `c`
    // (a `&mut` borrow) cannot alias A, the prepacked B lives in its own owned buffer, and the
    // bias was checked disjoint from C above. The core `_unchecked` tier itself raises the
...
/// from [`prepack_lhs`]
///
/// This is the nalgebra adapter over gemmkit's [`gemmkit::gemm_packed_a_fused`]. The same
/// [`PackedLhs`] serves both [`gemm_packed_a`] and this fused entry. `C` must be row-major-ish
/// (`|col stride| <= |row stride|`), for the same reason as [`gemm_packed_a`]. The optional
/// [`Bias`] is [`Bias::PerRow`] (length `A.rows`) or [`Bias::PerCol`] (length `B.cols`), given in
/// the user's frame, and the optional [`Activation`] is applied last. `bias == None && act ==
/// None` behaves exactly like [`gemm_packed_a`]
///
/// # Panics
///
/// If the dimensions disagree, if `C` is not row-major-ish, or on a bias or activation the
/// adapter rejects: a wrong-length bias, a bias slice that overlaps `C`, or a non-finite
/// `LeakyRelu` slope
#[cfg(feature = "epilogue")]
#[allow(clippy::too_many_arguments)]
    ...
    SC: RawStorageMut<T, RC, CC>,
{
    ...
    // Checks the bias length against the caller's axis (PerRow == packed A.rows == C.rows,
    // PerCol == B.cols) and rejects an overlap with C, then a finite LeakyRelu slope, matching
    // the core checked entry's wording. This adapter passes the bias down in the user's frame,
    // and the core `gemm_packed_a_fused` flips its axis to match the transposed computation it
    // drives
    let (bias_ptr, bias_dim, has_bias) =
    ...
    // SAFETY: dims are checked above. nalgebra guarantees B/C layouts are valid in-bounds, `c`
    // (a `&mut` borrow) cannot alias B, the prepacked A lives in its own owned buffer, and the
    // bias was checked disjoint from C above. The core `_unchecked` tier itself raises the
