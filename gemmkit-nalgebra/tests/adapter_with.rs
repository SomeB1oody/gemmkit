...
fn gemm_fused_with_matches_gemm_fused() {
    use gemmkit_nalgebra::{Activation, Bias, gemm_fused, gemm_fused_with};
    let (m, k, n) = (12usize, 9, 7);
    let a = rand2(m, k, 401);
    let b = rand2(k, n, 402);
...
fn gemm_i8_requant_with_matches() {
    ...
    for &par in &[Parallelism::Serial, Parallelism::Rayon(0)] {
        ...
        let mut c_with = DMatrix::from_element(m, n, 0i8);
        gemm_i8_requant_with(&mut ws, &a, &b, mk(zp_i8), &mut c_with, par);
        let mut c_ref = DMatrix::from_element(m, n, 0i8);
        gemm_i8_requant(&a, &b, mk(zp_i8), &mut c_ref, par);
        assert_eq!(c_with, c_ref);

        let mut cu_with = DMatrix::from_element(m, n, 0u8);
        gemm_i8_requant_u8_with(&mut ws, &a, &b, mk(zp_u8), &mut cu_with, par);
        let mut cu_ref = DMatrix::from_element(m, n, 0u8);
...
fn gemm_cplx_fused_with_matches() {
    use gemmkit::Complex;
    use gemmkit_nalgebra::{Bias, gemm_cplx_fused, gemm_cplx_fused_with};
