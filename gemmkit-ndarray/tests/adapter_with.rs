...
fn gemm_fused_with_matches_gemm_fused() {
    use gemmkit_ndarray::{Activation, Bias, gemm_fused, gemm_fused_with};
    let (m, k, n) = (12usize, 9, 7);
    let a = rand2(m, k, 401);
    let b = rand2(k, n, 402);
...
fn gemm_batched_fused_with_matches() {
    use gemmkit_ndarray::{Activation, Bias, gemm_batched_fused, gemm_batched_fused_with};
    let (batch, m, k, n) = (4usize, 8, 8, 8);
    let a = rand3(batch, m, k, 411);
    let b = rand3(batch, k, n, 412);
    let c0 = rand3(batch, m, n, 413);
    let bias: Vec<f64> = (0..m).map(|i| 0.2 * i as f64 - 0.5).collect();
    let (alpha, beta) = (0.9f64, 1.2);
    let mut ws = Workspace::new();
    for &par in &[Parallelism::Serial, Parallelism::Rayon(0)] {
        let mut c_with = c0.clone();
        ...
        for e in 0..batch {
            for i in 0..m {
                for j in 0..n {
                    assert_eq!(
                        c_with[(e, i, j)].to_bits(),
                        c_ref[(e, i, j)].to_bits(),
                        "batched_fused_with ({e},{i},{j})"
                    );
                }
            }
        }
...
fn gemm_i8_requant_with_matches() {
    ...
    for &par in &[Parallelism::Serial, Parallelism::Rayon(0)] {
        ...
        let mut c_with = Array2::<i8>::zeros((m, n));
        gemm_i8_requant_with(&mut ws, &a, &b, mk(zp_i8), &mut c_with, par);
        let mut c_ref = Array2::<i8>::zeros((m, n));
        gemm_i8_requant(&a, &b, mk(zp_i8), &mut c_ref, par);
        assert_eq!(c_with, c_ref);

        let mut cu_with = Array2::<u8>::zeros((m, n));
        gemm_i8_requant_u8_with(&mut ws, &a, &b, mk(zp_u8), &mut cu_with, par);
...
fn gemm_cplx_fused_with_matches() {
    use gemmkit::Complex;
    use gemmkit_ndarray::{Bias, gemm_cplx_fused, gemm_cplx_fused_with};
