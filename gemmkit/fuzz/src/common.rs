...
/// `tests/correctness/mixed.rs` (e.g. `correctness_f16_layouts`). `0.0` is 1st so the
...
/// would truncate to `0` and collapse half of it - a dedicated integer table is used instead
...
/// interleave (`il`) x trailing-pad (`pad`) family. `BroadcastRow` (`rs = 0`) is a
...
impl LayoutPlan {
    /// `(rs, cs)` for a `rows x cols` view. Mirrors the extent formula of
...
/// Highest slice offset (exclusive) of a non-negative-stride view: mirrors
...
/// Allocate exactly the extent a `rows x cols` view needs, fill its view slots
...
/// Assert the driver never wrote a slot outside the `rows x cols` view: the cheapest
