...
//! another: the input/accumulator/output types, the pack layout, the microkernel,
//! and the epilogue. [`float::FloatGemm`] is the baseline family; complex,
//! integer, and mixed-precision GEMM are separate families that reuse the driver,
//! packing framework, cache model, and parallelism unchanged - that open/closed
//! property is part of the architecture contract and is proven by a test that
//! declares a 2nd trivial family
//!
//! The tile geometry (`MR_REG`, `NR`) is not on this trait: it is a pair of
...
// Complex GEMM family: split (SoA) kernel for Complex<f32>/Complex<f64>, with
// optional conjugation of A/B (requires the `complex` feature)
#[cfg(feature = "complex")]
pub mod complex;
// Fused epilogue trait and the built-in bias/activation and requantize epilogues
pub mod epilogue;
// The floating-point GEMM family: the single generic microkernel used by every ISA
pub mod float;
// Integer GEMM family: i8 inputs with an i32 accumulator, widen-and-multiply and
// VNNI dot variants (requires the `int8` feature)
#[cfg(feature = "int8")]
pub mod int;
// Mixed-precision GEMM family: f16/bf16 inputs with an f32 accumulator (requires
// the `half` feature)
#[cfg(feature = "half")]
pub mod mixed;
...
pub trait KernelFamily: Copy + Send + Sync + 'static {
    ...
    /// addresses the packed A/B buffers. `1` (the default) means no rounding: the
    /// case for every homogeneous / widen family. A dot-product family that folds
    /// `Q` consecutive depth steps into one instruction (VNNI `vpdpbusd`: `Q = 4`;
    /// `vdpbf16ps`: `Q = 2`) sets it to `Q`, so each micropanel holds whole
    ...
    /// depth-padding the tail (pad value is family-specific: VNNI pads A with its
    /// `+128` bias of zero, B with zero). The driver strides packed panels by the
    /// same padded depth, keeping them in lockstep. `Q = 1` is the current
    /// no-padding behavior, so existing families are unaffected
    ///
    /// [`pack_lhs`]: KernelFamily::pack_lhs
    /// [`pack_rhs`]: KernelFamily::pack_rhs
    const DEPTH_MULTIPLE: usize = 1;

    /// Pack an `mc x kc` LHS block into micropanel-major layout: a sequence of
    ...
    ///
    /// # Safety
    /// `src` must be valid for the `mc x kc` region described by `rs`/`cs`, and
    ...
    /// Pack a `kc x nc` RHS block into micropanel-major layout: panels each `nr`
    /// columns wide, stored row-by-row with `nr` contiguous columns per row
    /// (tail columns zero-filled)
    ///
    /// # Safety
    /// `src` valid for the `kc x nc` region; `dst` valid for
    ...
    /// Compute one `MR x NR` tile (`MR == MR_REG*LANES`, `LANES` the `Acc` lane count)
    /// and apply the epilogue `C <- combine(alpha*A*B, beta*C)`
    ///
    /// # Parameters
    /// - `a`/`a_cs` - LHS panel base and column (depth) stride; packed input has
    ///   `a_cs == mr`, adaptive (unpacked) column-major input has `a_cs == csa`; LHS
    ///   rows are always unit-stride
    ...
    ///
    /// A family implements exactly one of the 2 microkernel methods. A non-fusing
    /// family overrides this plain kernel and inherits the default [`microkernel_epi`],
    /// which forwards straight here (carrying the free fail-closed
    /// `assert!(E::IS_IDENTITY)`): `IntGemm`, `IntGemmVnni`, and the open/closed test
    /// family take this path. A fusing family instead overrides [`microkernel_epi`] to
    /// thread the epilogue `E` through its own store, and this plain method is then
    /// never called, so the default body here is `unreachable!`. (`FloatGemm` and the
    /// mixed / requant families override `microkernel_epi`; `ComplexGemm`'s override
    /// happens to also call `Self::microkernel`, so it provides both)
    ///
    /// # Safety
    ...
    /// Compute one `MR x NR` tile and apply the fused [`Epilogue`] `E` to each stored
    /// element. `row0`/`col0` are the tile's origin in the oriented problem frame (so
    /// a per-row/per-col bias resolves its absolute base), and `last_k` is whether this
    /// is the final depth panel: the epilogue applies only then for `OUT_IS_ACC`
    /// families (intermediate panels store raw `Acc` partials; see [`KernelFamily`])
    ///
    /// The default forwards to [`KernelFamily::microkernel`], ignoring the epilogue
    /// arguments: it is correct only for `E = Identity` (a `debug_assert` enforces
    /// this), which is exactly what the driver passes on every non-fused path, so
    /// `mixed`, `complex`, and the `i32`-out integer families need no edit and stay
    /// byte-for-byte unchanged. A family that supports fusion (the float family and
    /// the requantizing integer families) overrides this to thread `E` through its
    /// store
    ///
    /// # Safety
