...
//! single work-gate, no nested 2x2 tree. Demand-driven pulling means faster cores
...
//! a fixed config gives the same result. (That the 2 are also bitwise-equal today
...
fn auto_threads() -> usize {
    match std::thread::available_parallelism() {
        Ok(n) => n.get(),
        // wasm has no `available_parallelism`, so fall back to the tunable
        // wasm worker count - reached only under `RAYON_USABLE`
    ...
    pub(crate) fn resolve(self, mnk: usize, n_jobs: usize) -> usize {
        ...
        match self {
            ...
            Parallelism::Rayon(req) => {
                ...
                // `Rayon(huge)` can't over-subscribe or over-allocate pack
                // regions. Only auto (below) is heuristic, so `Rayon(n)` gives
                // the exact width the tests and scaling diagnostic ask for
                if req != 0 {
...
impl Parallelism {
    /// Pick the batched schedule for `batch` products of shape `m x k x n` (`sizeof` bytes/element).
    ...
    pub(crate) fn resolve_batch(
        ...
        match self {
            ...
            Parallelism::Rayon(req) => {
                ...
                // faster for 512^3 b<=8, 384^3 b<=4, 256^3 b<=4, and one-per-worker for the small
                // rest (256^3 b=8). The 1-D share rule is approximate - it also splits 384^3 b8 (a
                // ~1.06x miss) - but the old `elem_bytes > l2.effective` (3.2 MiB) missed *all* the
                // wins, so 512^3 ran BatchParallel(2) at 0.28x of the per-element split
...
///
/// * **x86**: a *quarter* (divide by 2 for SMT, divide by 2 again because roughly half the
///   physical cores saturate DDR). Calibrated on Zen5, where a bandwidth-bound gemv plateaus
///   around a quarter of the 32 logical cores
/// * **aarch64** (Apple/ARM, no SMT): a *half* - drop the divide-by-2-for-SMT factor.
///   Calibrated on M4 Max (10P+4E, ~245 GB/s aggregate): a bandwidth-bound gemv climbs to
///   about 8 of 14 workers and then *declines* (the E-cores add contention, not bandwidth),
