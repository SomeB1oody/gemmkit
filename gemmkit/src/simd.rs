...
//! is enabled. CPU support is decided at *runtime* (by the dispatch layer), so a
//! fixed `#[target_feature]` cannot go on the generic kernel. Instead each
...
// Split (SoA) complex microkernel and the per-ISA glue that exposes it
#[cfg(feature = "complex")]
#[macro_use]
mod complex;
...
mod neon;
// Scalar (1-lane) ISA token: the portability floor and Miri-checked reference path
mod scalar;
...
pub trait KernelSimd<L: Scalar, R: Scalar, A: Scalar, O: Scalar>: SimdOps<A> {
    ...
    /// Accumulate one full `MR_REG x NR` microtile from **dot-product**-packed panels into
    ...
    /// internally so `acc` holds the true `sum_k(A*B)` on return
    ///
...
/// neither the homogeneous blanket (all 4 types equal) nor the sibling byte blanket (distinct
/// `Out`)
#[cfg(feature = "int8")]
...
pub trait SimdOps<T: Scalar>: Simd {
    ...
    /// step's single rounding so the 2 interleave consistently
    ///
    /// # Safety
    /// See the trait-level note
    ...
    /// Accumulate one **full** `MR_REG x NR` microtile over `kc` depth steps into
    ...
    /// # Safety
    ///
    /// `a` valid for `MR_REG*LANES` rows x `kc` depth at stride `a_cs`; `b` valid
    /// for `NR` cols x `kc` depth at strides `b_rs`/`b_cs`; `acc` pre-initialized.
    ...
    /// Compute one `MR x NR` **complex** tile in the split (structure-of-arrays) layout
