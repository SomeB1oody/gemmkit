...
/// split the 16 `i32` into 2 `__m256i` halves, requantize each in `f64`
...
unsafe fn requant_store_avx512(dst: *mut i8, v: __m512i, scale: f64, zp: i32, lo: i32, hi: i32) {
    unsafe {
        ...
        // Recombine the 2 8-lane halves into one 16-lane `__m512i`, then truncate each lane
...
impl Simd for Avx512Vnni {
    ...
    unsafe fn vectorize<R>(self, f: impl FnOnce() -> R) -> R {
        ...
        // dispatcher verifies all 3 before selecting this token
...
impl KernelSimd<i8, i8, i32, i32> for Avx512Vnni {
    ...
    unsafe fn dot_accumulate<const MR_REG: usize, const NR: usize>(
        ...
        unsafe {
            ...
            // summing the padded panel equals summing real B. B is broadcast, so every lane
            // shares the same `colsum[j]`; a scalar sum (mod 2^32, matching the i32
            // accumulation) suffices. (`s` over one quad is bounded by `4*127`, no overflow;
            // the running `colsum` wraps.) This recomputes the sum once per A row-panel
            // rather than once per B panel; the scalar pass is a small fraction of the
            // `vpdpbusd` work and the B strip is cache-warm, so it is kept here instead of
            // widening the packed-panel layout (and the driver's buffer sizing) to carry a
            // precomputed column sum
            ...
            // `i` is 16 rows x 4 contiguous k-bytes (64 B); a B quad broadcasts 4
            ...
            // to recover the true signed `sum_k(A*B)`. Same bias the LHS pack offsets by
            for j in 0..NR {
                let corr =
...
impl KernelSimd<bf16, bf16, f32, bf16> for Avx512Bf16 {
    ...
    unsafe fn dot_accumulate<const MR_REG: usize, const NR: usize>(
        ...
        unsafe {
            ...
            // `f32(a0)*f32(b0) + f32(a1)*f32(b1)`. A register `i` holds 16 rows x 2
