...
/// The split (SoA) complex microkernel: accumulate one `MR × NR` complex tile in
/// separate real/imag register banks, then apply the complex `alpha`/`beta` epilogue.
///
/// One complex multiply-accumulate is four fused real steps into two banks —
...
/// `mr_eff × nr_eff` output sub-tile at `rsc`/`csc`; `scratch` valid for `2*mr*NR`
    ...
    S: SimdOps<C::Real>,
{
    unsafe {
        ...
        // `fnma` into `acc_re`; two `mul_add`s into `acc_im`). This fixed per-`p` order is
