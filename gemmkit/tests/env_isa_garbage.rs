...
//! other tests. The unknown-value arm of the pin parser fires the first time any `select_*` ladder
...
fn garbage_pin_is_a_hard_error() {
    ...
    let mut c = [0.0f32; 4];
    // The first dispatch resolves the pin and panics on the unknown-value arm.
    gemmkit::gemm(
        1.0,
